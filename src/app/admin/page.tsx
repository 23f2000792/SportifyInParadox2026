          ...
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {EVENTS.filter(e => isSuperAdmin || adminProfile.assignedSport === 'all' || e.slug === adminProfile.assignedSport).map((event) => {
                ...
                return (
                  <Button key={event.id} variant="ghost" className="p-0 h-auto text-left w-full" onClick={() => setSelectedSportSlug(event.slug)}>
                    <Card className="premium-card w-full min-h-[80px] py-4 flex items-center px-4 sm:px-6 gap-3 sm:gap-6 hover:bg-muted/10 overflow-hidden">
                      ...
                      <div className="min-w-0 flex-1">
                        <h2 className="text-xs sm:text-sm md:text-lg font-black uppercase text-foreground tracking-tight line-clamp-2 leading-tight">
                          {event.name}
                        </h2>
