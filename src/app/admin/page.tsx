...
export default function AdminPage() {
  ...
  const handleUpdateMatch = (e: React.FormEvent) => {
    ...
    toast({ title: "Live Protocol Updated" });
  };
  ...
  const handleCreateOrUpdateSchedule = (e: React.FormEvent) => {
    ...
    if (editMatchId) {
      ...
      toast({ title: "Transmission Initialized" });
    }
    // Reset
  ...
  const handleSaveRunSchedule = async (e: React.FormEvent) => {
    ...
    toast({ title: "Race Timings Synced" });
  };
  ...
  const handleAddStanding = (e: React.FormEvent) => {
    ...
    setNewStandingTeam('');
    toast({ title: "House Enrolled in Group" });
  };
  ...
  const handleAddRunResult = (e: React.FormEvent) => {
    ...
    toast({ title: "Rank Record Archived" });
  };
      ...
      <div className="space-y-12 max-w-5xl mx-auto py-10">
        <div className="flex justify-between items-center border-b border-white/5 pb-8">
          <div className="space-y-2">
            <h1 className="text-5xl font-black italic tracking-tighter uppercase leading-none">Command Hub</h1>
            <p className="text-[10px] font-black uppercase tracking-[0.5em] text-primary">Security Clearance Level: {adminProfile.role}</p>
          </div>
          <Button variant="ghost" size="sm" onClick={() => signOut(auth)} className="bg-destructive/10 text-destructive hover:bg-destructive/20 text-[9px] font-black uppercase tracking-widest gap-2">
          ...
          {EVENTS.map((event) => {
            ...
            return (
              <Button key={event.id} variant="ghost" className="p-0 h-auto group" onClick={() => setSelectedSportSlug(event.slug)}>
                <Card className="premium-card w-full h-48 border-white/5 hover:border-primary/50 transition-all duration-500 overflow-hidden">
                  <CardContent className="p-0 flex h-full">
                    <div className="w-1/3 bg-primary/10 flex items-center justify-center border-r border-white/5 group-hover:bg-primary/20">
                    ...
                    <div className="w-2/3 p-8 flex flex-col justify-center text-left space-y-2">
                      ...
                      <div className="pt-4 flex items-center text-[10px] font-black uppercase text-primary gap-2 opacity-0 group-hover:opacity-100 transition-opacity">Launch Protocols <ChevronRight className="h-3 w-3" /></div>
                    </div>
                  </CardContent>
                </Card>
              </Button>
        ...
        {isSuperAdmin && (
          <Card className="premium-card border-primary/20 bg-primary/5 mt-10">
            <CardHeader><CardTitle className="text-xs font-black uppercase tracking-[0.3em] flex items-center gap-2"><ShieldCheck className="h-4 w-4" /> Global Systems Access</CardTitle></CardHeader>
            <CardContent>
              <Button variant="secondary" className="w-full uppercase font-black text-[10px] tracking-widest" onClick={() => { setSelectedSportSlug('football'); setActiveTab('access'); }}>Personnel Matrix Override</Button>
            </CardContent>
          </Card>
        )}
      </div>
    ...
    <div className="max-w-6xl mx-auto space-y-8 pb-20">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 border-b border-white/5 pb-8">
        <div className="space-y-2">
          ...
          <h1 className="text-4xl font-black italic uppercase flex items-center gap-4 tracking-tighter">{currentSport?.name} Dashboard</h1>
          <p className="text-[10px] text-muted-foreground uppercase font-black tracking-[0.3em]">Operator Identity: {adminProfile.email}</p>
        </div>
        <div className="flex items-center gap-4">
           <Badge variant="outline" className="h-8 border-primary/20 text-primary uppercase font-black text-[10px] bg-primary/5 tracking-widest px-4">Live Link Active</Badge>
        </div>
      </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-8">
        <TabsList className={cn(
          ...
          <TabsTrigger value="schedule" className="text-[10px] font-black uppercase rounded-xl">Schedule</TabsTrigger>
          {!isKampusRun && <TabsTrigger value="standings" className="text-[10px] font-black uppercase rounded-xl">League Table</TabsTrigger>}
          <TabsTrigger value="archive" className="text-[10px] font-black uppercase rounded-xl">Archives</TabsTrigger>
          {isSuperAdmin && <TabsTrigger value="access" className="text-[10px] font-black uppercase rounded-xl hidden md:flex">Security</TabsTrigger>}
        </TabsList>
              ...
              <CardContent className="p-8">
                <form onSubmit={handleAddRunResult} className="grid grid-cols-1 md:grid-cols-6 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input type="number" value={runnerPos} onChange={e => setRunnerPos(Number(e.target.value))} className="bg-white/5 h-14 border-white/10 text-xs font-black" />
                  </div>
                  <div className="space-y-2">
                    <Label className="text-[10px] font-black uppercase opacity-60 ml-1">Chrono Record</Label>
                    <Input placeholder="00:00.0" value={runnerTime} onChange={e => setRunnerTime(e.target.value)} className="bg-white/5 h-14 border-white/10 text-xs font-black" required />
                  </div>
                  <div className="space-y-2">
                  ...
                  <div className="space-y-2">
                    <Label className="text-[10px] font-black uppercase opacity-60 ml-1">Division</Label>
            ...
            <Card className="premium-card border-white/5">
              <CardContent className="p-8 space-y-10">
                <div className="space-y-3">
                  <Label className="text-[10px] font-black uppercase opacity-50 tracking-[0.3em] ml-1">Target Transmission Match</Label>
                ...
                {selectedMatchId && (
                  <form onSubmit={handleUpdateMatch} className="space-y-12 animate-in fade-in duration-500">
                    <div className="flex items-center justify-between gap-6 md:gap-12">
                      <div className="flex-1 space-y-4">
                        <Label className="text-[10px] font-black uppercase text-center block opacity-40">{activeMatch?.teamA}</Label>
                        <Input type="number" value={scoreA} onChange={e => setScoreA(Number(e.target.value))} className="text-center text-5xl md:text-7xl font-black h-24 md:h-32 bg-white/5 border-white/10 rounded-3xl" />
                      </div>
                      <div className="text-4xl font-black opacity-10 pt-10">:</div>
                      <div className="flex-1 space-y-4">
                        <Label className="text-[10px] font-black uppercase text-center block opacity-40">{activeMatch?.teamB}</Label>
                        <Input type="number" value={scoreB} onChange={e => setScoreB(Number(e.target.value))} className="text-center text-5xl md:text-7xl font-black h-24 md:h-32 bg-white/5 border-white/10 rounded-3xl" />
                      </div>
                    </div>
                    {selectedSportSlug === 'badminton' && (
                      <div className="space-y-6 pt-10 border-t border-white/5">
                        <h4 className="text-[10px] font-black uppercase tracking-widest text-primary/60">Sub-Match Breakdown Matrix</h4>
                        <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
                          {badmintonResults.map((res, idx) => (
                            <div key={idx} className="bg-white/5 p-5 rounded-2xl border border-white/5 space-y-4">
                              ...
                              <Input value={res.score} onChange={e => { const n = [...badmintonResults]; n[idx].score = e.target.value; setBadmintonResults(n); }} className="h-10 text-xs font-black text-center bg-black/20" placeholder="0-0" />
                              <Select value={res.winner} onValueChange={v => { const n = [...badmintonResults]; n[idx].winner = v; setBadmintonResults(n); }}>
                                <SelectTrigger className="h-8 text-[8px] font-black uppercase bg-transparent"><SelectValue placeholder="Winner" /></SelectTrigger>
                                <SelectContent>
                    ...
                    <div className="space-y-4">
                      <Label className="text-[10px] font-black uppercase opacity-50 tracking-widest ml-1">Protocol State</Label>
                      <Select value={status} onValueChange={(v: any) => setStatus(v)}>
                        ...
                        <SelectContent>
                          ...
                          <SelectItem value="Completed" className="text-[10px] font-black uppercase">Archive Archive</SelectItem>
                        </SelectContent>
                      </Select>
                    </div>
                    <Button type="submit" className="w-full h-16 font-black uppercase text-xs tracking-[0.3em] gap-4 shadow-2xl shadow-primary/30 rounded-2xl">Push Live Update</Button>
                  </form>
                )}
              </CardContent>
            </Card>
        ...
        <TabsContent value="schedule" className="space-y-8">
           {isKampusRun ? (
             <Card className="premium-card border-white/5">
               <CardHeader className="bg-primary/5 border-b border-white/5"><CardTitle className="text-xs font-black uppercase italic tracking-widest text-primary">Race Day Scheduling Domain</CardTitle></CardHeader>
               <CardContent className="p-8">
                 <form onSubmit={handleSaveRunSchedule} className="space-y-12">
                   <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                      <div className="space-y-2"><Label className="text-[10px] font-black uppercase tracking-widest opacity-60">Competition Date</Label><Input type="date" value={runDate} onChange={e => setRunDate(e.target.value)} className="bg-white/5 h-14 font-black text-xs" /></div>
                   </div>
                   <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                      <div className="p-6 rounded-2xl bg-white/[0.02] border border-white/5 space-y-4">
                        <h4 className="text-[11px] font-black uppercase text-primary italic">3KM Category</h4>
                        <div className="grid grid-cols-1 gap-4">
                          ...
                          <div className="space-y-2"><Label className="text-[9px] font-black uppercase opacity-40">Start Vector</Label><Input value={run3kStart} onChange={e => setRun3kStart(e.target.value)} placeholder="06:30 AM" className="bg-white/5 h-12 font-black text-xs" /></div>
                        </div>
                      </div>
                      <div className="p-6 rounded-2xl bg-white/[0.02] border border-white/5 space-y-4">
                        <h4 className="text-[11px] font-black uppercase text-primary italic">5KM Category</h4>
                        <div className="grid grid-cols-1 gap-4">
                          ...
                          <div className="space-y-2"><Label className="text-[9px] font-black uppercase opacity-40">Start Vector</Label><Input value={run5kStart} onChange={e => setRun5kStart(e.target.value)} placeholder="06:00 AM" className="bg-white/5 h-12 font-black text-xs" /></div>
                        </div>
                      </div>
                   </div>
                   <Button type="submit" className="w-full h-16 uppercase font-black tracking-[0.3em] rounded-2xl shadow-xl shadow-primary/20">Archive Race Protocol</Button>
                 </form>
               </CardContent>
             </Card>
           ) : (
            <Card className="premium-card border-white/5">
              <CardHeader className="bg-white/[0.02] border-b border-white/5"><CardTitle className="text-xs font-black uppercase italic tracking-widest">{editMatchId ? "Modify Transmission Record" : "Initialize New Match"}</CardTitle></CardHeader>
              <CardContent className="p-8">
                <form onSubmit={handleCreateOrUpdateSchedule} className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="space-y-2"><Label className="text-[10px] font-black uppercase tracking-widest opacity-60">Match Index (M#)</Label><Input value={schedMatchNumber} onChange={e => setSchedMatchNumber(e.target.value)} placeholder="e.g., 101" className="bg-white/5 h-14 font-black text-xs" required /></div>
                  <div className="space-y-2"><Label className="text-[10px] font-black uppercase tracking-widest opacity-60">Phase Vector</Label><Select value={schedPhase} onValueChange={(v: any) => setSchedPhase(v)}><SelectTrigger className="bg-white/5 h-14 font-black text-[11px] uppercase"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="group" className="text-[10px] font-black uppercase">Group Stage</SelectItem><SelectItem value="semi-final" className="text-[10px] font-black uppercase">Semi Final</SelectItem><SelectItem value="third-place" className="text-[10px] font-black uppercase">3rd Place Play-off</SelectItem><SelectItem value="final" className="text-[10px] font-black uppercase">Grand Final</SelectItem></SelectContent></Select></div>
                  ...
                  {(selectedSportSlug === 'badminton' || selectedSportSlug === 'volleyball') && (<div className="space-y-2"><Label className="text-[10px] font-black uppercase tracking-widest opacity-60">Court Number</Label><Input value={schedCourtNumber} onChange={e => setSchedCourtNumber(e.target.value)} placeholder="e.g., Court 1" className="bg-white/5 h-14 font-black text-xs" /></div>)}
                  {selectedSportSlug === 'football' && (<div className="space-y-2"><Label className="text-[10px] font-black uppercase tracking-widest opacity-60">Ground Index</Label><Input value={schedGroundNumber} onChange={e => setSchedGroundNumber(e.target.value)} placeholder="e.g., Ground A" className="bg-white/5 h-14 font-black text-xs" /></div>)}
                  <Button type="submit" className="md:col-span-2 h-16 uppercase font-black text-xs tracking-[0.3em] gap-4 shadow-xl shadow-primary/20 rounded-2xl mt-4">{editMatchId ? <Save className="h-6 w-6" /> : <Plus className="h-6 w-6" />}{editMatchId ? "Authorize Modification" : "Confirm Schedule Sync"}</Button>
                </form>
              </CardContent>
            </Card>
           )}
           ...
           <div className="space-y-6">
             <h3 className="text-[11px] font-black uppercase tracking-[0.4em] text-muted-foreground flex items-center gap-3 ml-2"><Activity className="h-4 w-4 text-primary" /> Transmission Queue</h3>
             <div className="grid grid-cols-1 gap-4">
               {matches?.map(match => (
                 <Card key={match.id} className="premium-card group hover:border-primary/30 transition-all bg-white/[0.01]">
                   <CardContent className="p-6 flex flex-col md:flex-row items-center justify-between gap-6">
                     ...
                     <div className="flex items-center gap-3">
                       <Button size="icon" variant="ghost" className="h-10 w-10 text-muted-foreground hover:text-primary" onClick={() => {
                       ...
                       <Button size="icon" variant="ghost" className="h-10 w-10 text-muted-foreground hover:text-destructive" onClick={() => deleteDoc(doc(db!, 'matches', match.id))}><Trash2 className="h-5 w-5" /></Button>
                     </div>
                   </CardContent>
                 </Card>
               ))}
             ...
             <Card className="premium-card border-white/5">
               ...
               <CardContent className="p-8">
                 <form onSubmit={handleAddStanding} className="grid grid-cols-1 md:grid-cols-3 gap-6 items-end">
                   <div className="space-y-2"><Label className="text-[10px] font-black uppercase opacity-60">House Domain</Label><Select value={newStandingTeam} onValueChange={setNewStandingTeam}><SelectTrigger className="bg-white/5 h-12 text-xs font-black uppercase"><SelectValue placeholder="Select House" /></SelectTrigger><SelectContent>{HOUSES.map(h => <SelectItem key={h} value={h} className="text-[10px] font-black uppercase">{h}</SelectItem>)}</SelectContent></Select></div>
                   <div className="space-y-2"><Label className="text-[10px] font-black uppercase opacity-60">Group</Label><Select value={newStandingGroup} onValueChange={setNewStandingGroup}><SelectTrigger className="bg-white/5 h-12 text-xs font-black uppercase"><SelectValue /></SelectTrigger><SelectContent>{GROUPS.map(g => <SelectItem key={g} value={g} className="text-[10px] font-black uppercase">Group {g}</SelectItem>)}</SelectContent></Select></div>
                   <Button type="submit" className="h-12 uppercase font-black text-[10px] tracking-widest"><Plus className="h-4 w-4 mr-2" /> Assign to Group</Button>
                 </form>
               </CardContent>
             </Card>
             <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
