...
import { Trophy, Timer, Zap, CircleDot, Target, User, Clock, Medal, MapPin, Hash, Calendar } from 'lucide-react';
      ...
      {sport === 'kampus-run' ? (
        <section className="space-y-8">
          ...
          <Card className="premium-card">
            <Table>
              <TableHeader className="bg-white/5"><TableRow className="border-white/5"><TableHead className="w-16 text-center text-[9px] font-black uppercase">Pos</TableHead><TableHead className="text-[9px] font-black uppercase">Participant</TableHead><TableHead className="text-[9px] font-black uppercase">Category</TableHead><TableHead className="text-right text-[9px] font-black uppercase">Time</TableHead></TableRow></TableHeader>
              <TableBody>
                {runResults?.map((res) => (
                  <TableRow key={res.id} className="h-14 border-white/5 hover:bg-white/[0.02] transition-colors">
                    ...
                    <TableCell><p className="font-bold text-sm uppercase">{res.name}</p><p className="text-[8px] font-black text-muted-foreground uppercase">{res.gender} • {res.ageGroup}</p></TableCell>
                  ...
                  <Card key={match.id} className="premium-card border-live/30">
                    <CardContent className="p-8">
                      <div className="flex flex-col md:flex-row items-center justify-between gap-8">
                        <div className="flex-1 text-center md:text-right"><p className="text-2xl font-black uppercase italic tracking-tighter">{match.teamA}</p></div>
                        <div className="flex flex-col items-center gap-4">
                          <div className="text-6xl font-black tabular-nums tracking-tighter bg-white/5 px-10 py-4 rounded-3xl border border-white/10 shadow-2xl">{match.scoreA} : {match.scoreB}</div>
                          <Badge className="bg-live text-black text-[9px] font-black uppercase animate-pulse">Transmission Active</Badge>
                        </div>
                        <div className="flex-1 text-center md:text-left"><p className="text-2xl font-black uppercase italic tracking-tighter">{match.teamB}</p></div>
                      </div>
                      ...
                      <div className="mt-8 text-center border-t border-white/5 pt-4 flex flex-col items-center gap-1">
                        <span className="text-[9px] font-black uppercase text-primary/60 tracking-[0.2em]">Match #{match.matchNumber}</span>
                        <span className="text-[9px] font-black uppercase text-muted-foreground/40 tracking-[0.3em] flex items-center gap-2">
                          <MapPin className="h-3 w-3" /> {match.venue} {match.courtNumber || match.groundNumber ? `• ${match.courtNumber || match.groundNumber}` : ''}
                        </span>
                      </div>
                    </CardContent>
                  </Card>
                  ...
                  <Card key={match.id} className="premium-card group border-white/5">
                    <CardContent className="p-6 flex items-center justify-between">
                      <div className="flex items-center gap-6">
                        <div className="text-center pr-6 border-r border-white/5 w-24">
                           <p className="text-[8px] font-black text-primary/60 uppercase">M#{match.matchNumber}</p>
                           <p className="text-xs font-black text-white uppercase">{match.time}</p>
                           <p className="text-[7px] font-black text-muted-foreground/40 uppercase">{match.day}</p>
                        </div>
                        <div>
                          ...
                          <div className="flex items-center gap-3 mt-1">
                            <span className="text-[8px] font-black text-muted-foreground/40 uppercase flex items-center gap-1"><MapPin className="h-2 w-2" /> {match.venue}</span>
                            <span className="text-[8px] font-black text-muted-foreground/40 uppercase flex items-center gap-1"><Calendar className="h-2 w-2" /> {match.date}</span>
                            { (match.courtNumber || match.groundNumber) && <span className="text-[8px] font-black text-primary/40 uppercase">Loc: {match.courtNumber || match.groundNumber}</span> }
                          </div>
                        </div>
                      </div>
                    </CardContent>
                  ...
                  <Card key={match.id} className="premium-card group border-white/5">
                    <CardContent className="p-0">
                      <div className="p-6 flex items-center justify-between">
                        ...
                        <div className="text-2xl font-black tabular-nums bg-white/5 px-6 py-2 rounded-xl border border-white/10">{match.scoreA} - {match.scoreB}</div>
                      ...
                      <div className="flex items-center justify-between px-6 py-3 border-t border-white/5 bg-white/[0.01]">
                        <span className="text-[8px] font-black uppercase text-muted-foreground/30 tracking-widest">Match #{match.matchNumber} • {match.venue} • {match.phase}</span>
