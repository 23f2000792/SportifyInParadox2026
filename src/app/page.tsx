...
import { Trophy, Zap, CircleDot, Target, ChevronRight, Activity, Radio, MapPin } from 'lucide-react';
          ...
          <div className="grid grid-cols-1 gap-4">
            {liveMatches.map((match) => (
              <Link key={match.id} href={`/events/${match.sport}`}>
                <Card className="premium-card border-primary/30 bg-primary/5 hover:bg-primary/10 group">
                  <CardContent className="p-5 flex items-center justify-between">
                    <div className="space-y-2">
                      ...
                      <p className="text-lg md:text-xl font-black uppercase italic tracking-tighter text-white">
                        {match.teamA} <span className="text-primary/60 px-2">{match.scoreA} : {match.scoreB}</span> {match.teamB}
                      </p>
                    </div>
                    ...
                    <div className="flex h-36 md:h-40">
                      ...
                      <div className="w-3/4 p-6 flex flex-col justify-center space-y-1.5">
                        ...
                        <div className="pt-2 flex items-center text-[10px] font-black uppercase text-primary gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                          Enter Stream <ChevronRight className="h-3 w-3" />
                        </div>
                      </div>
                    </div>
                  </CardContent>
